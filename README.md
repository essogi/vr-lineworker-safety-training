...
the run you get a report: time, tool drops, and any safety infractions — clamping
before checking, clamping out of order, working outside the designated zone.
...
Environment 2 has none). Details and results are in the
...
source (verbatim as shipped in May 2023), documentation, and build — not the full
Unity project, whose third-party assets can't be redistributed.

## Team & credits

Four-person capstone team (Texas A&M, CSCE Virtual Reality, spring 2023), in
partnership with TEEX:

- **Peter Castelein** — player-interaction layer: the shotgun stick, clamp/power-check
  sockets, and runtime lines (`Assets/Scripts/ShotgunRod/`), the infraction/scoring
  engine (`Tracker.cs`), instruction copy and experiment-flow edits, and the Quest
  builds.
- **Robin Schniebel** — bucket and truck controls, experiment panels, highlight
  system, sound, environment assembly, XR scaffolding.
